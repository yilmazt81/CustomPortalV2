    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (params.row.controlType == 'ComboBox' || params.row.controlType=='CheckBox' || params.row.controlType=='RadioBox')  {
          statusIcon = <IconButton key="combo" color="secondary" onClick={() => OptionClick('AddComboItem', params.row.id)} > <AddCircleIcon /></IconButton>;
        } else if (params.row.autoComplate === true) {
       
          statusIcon =<Link key="auto" to={{
            pathname: '/formdefinationsAComp',
            search: `?formfieldid=${params.row.id}`,
            }}> <AutoModeIcon /></Link>;
        }
        return (
          <React.Fragment key={`actions-${params.row.id}`}>
            <IconButton
